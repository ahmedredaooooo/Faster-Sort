...
namespace Problem
{
    ...
    public class Problem : ProblemBase, IProblem
    {
        ...
        protected override void RunOnSpecificFile(string fileName, HardniessLevel level, int timeOutInMillisec)
        {
            ...
            long expectedTime = -1;                           /// Ahmedreda
            Stopwatch AR = Stopwatch.StartNew();         /// Ahmedreda
            float avgTime = 0;
            float maxTime = 0;
            int i = 1;
            while (testCases-- > 0)
            {
                ...
                caseException = false;
                {
                    tstCaseThr = new Thread(() =>
                    {
                        try
                        {
                            ...
                            {
                                AR = Stopwatch.StartNew();                             /// Ahmedreda
                ...
                Console.WriteLine("Test Case {0}: timeOutInMillisec = {1} \t\t expected time = {2} ", i, timeOutInMillisec, AR.ElapsedMilliseconds);/// Ahmed Reda
